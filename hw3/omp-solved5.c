...
#pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid)
  ...
  #pragma omp sections nowait
    ...
    #pragma omp section
      ...
      for (i=0; i<N; i++)
        a[i] = i * DELTA;
      omp_unset_lock(&locka); //locka was not released before trying to acquire lockb. And the other thread keeps lockb and try to acquire locka. So deadlock.
      omp_set_lock(&lockb);   //It is probably the worst idea to keep two locks in this way. Sothe solution is to release the locks in time.
